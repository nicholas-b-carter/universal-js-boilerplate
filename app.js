...
var parseData = function parseData(str) {
    ...
    data.schedule.map(function (v, i) {
        v.start = new Date(v.start);
    });
    return data;
};

var byStartTime = function byStartTime(_ref, _ref2) {
    var a = _ref.start;
    var b = _ref2.start;
    return +a <= +b ? -1 : 1;
};
var findCurrent = function findCurrent(sortedSchedule) {
    var now = arguments.length <= 1 || arguments[1] === undefined ? new Date() : arguments[1];
    return sortedSchedule.reduce(function (a, v, i) {
        return +now >= +v.start ? v : a;
    });
};

var findPrev = function findPrev(sortedSchedule, current) {
    var now = arguments.length <= 2 || arguments[2] === undefined ? new Date() : arguments[2];
    return sortedSchedule[sortedSchedule.indexOf(current) - 1];
};

var findNext = function findNext(sortedSchedule, current) {
    var now = arguments.length <= 2 || arguments[2] === undefined ? new Date() : arguments[2];
    return sortedSchedule[sortedSchedule.indexOf(current) + 1];
};

var time = function time(d) {
    var h = arguments.length <= 1 || arguments[1] === undefined ? d.getHours() : arguments[1];
    var m = arguments.length <= 2 || arguments[2] === undefined ? d.getMinutes() : arguments[2];

    var suffix = h < 12 ? 'am' : 'pm';
    return (h === 12 ? 12 : h % 12) + ':' + (m < 10 ? '0' : '') + m + ' ' + suffix;
};

var setData = function setData(d) {
...
var talk = function talk(t) {
    ...
    if (!t) return '';
    // console.log(t)
    return m('.talk' + c, t.people && t.people.length ? [m('h1', m('span', t.people[0].name)), m('h4', m('span', t.people[0].twitter)), //, ' | ', t.people[0].github)),
    m('h2', m('span', t.title))] : [m('h1', m('span', t.title))]);
};

// m('h5', m('span', t.url)),
// m('h5', m('span', time(t.start)))
var sub = function sub(t) {
    var c = arguments.length <= 1 || arguments[1] === undefined ? '' : arguments[1];

    if (!t) return '';

    return m('.next' + c, m('span', 'Up next: '), m('h6', m('span', time(t.start)), t.people && t.people.length ? [' - ', m('span', t.people[0].name)] : [' - ', t.title]));
};

var handleKeypress = function handleKeypress(_ref3) {
    var which = _ref3.which;

    var delta = which === 37 ? -1 : which === 39 ? 1 : 0;
    navTo = Math.max(0, Math.min(data.schedule.length, (navTo || 0) + delta));
    if (delta !== 0) update();
};

var APPVIEW = function APPVIEW(m) {
    return function (prev, current, next, fadeOut) {
        ...
        // m('div', talk(prev)),
        talk(current), sub(next));
    };
};

var start = function start(data, view) {
    var update = arguments.length <= 2 || arguments[2] === undefined ? function () {} : arguments[2];
    var root = arguments[3];
    var mount = arguments[4];

    var nav = function nav(to) {
        var n = s[to || navTo];
    ...
    mount(function () {
        return data ? view(prev, nav(), next, fadeOut) : m('div');
    }, root);
};
...
},{"./data":2,"babel-polyfill":3,"universal-utils":307}],2:[function(require,module,exports){
'use strict';
...
},{"babel-regenerator-runtime":4,"core-js/fn/regexp/escape":5,"core-js/shim":299}],4:[function(require,module,exports){
...
},{"_process":301}],5:[function(require,module,exports){
...
},{"whatwg-fetch":317}],301:[function(require,module,exports){
...
},{"isomorphic-fetch":300}],305:[function(require,module,exports){
"use strict";
...
},{"./cache":302,"./csp":303,"./fetch":304,"./fp":305,"./hamt":306,"./lazy":308,"./meta":309,"./model":310,"./mux":311,"./ot":312,"./resource":313,"./router-alt":314,"./store":315,"./vdom":316}],308:[function(require,module,exports){
'use strict';
...
},{"./fetch":304,"./store":315}],312:[function(require,module,exports){
'use strict';
...
**/
},{"./fp":305}],313:[function(require,module,exports){
'use strict';
...
},{"./cache":302,"./fetch":304,"./store":315}],314:[function(require,module,exports){
'use strict';
...
*/
},{"./fetch":304}],316:[function(require,module,exports){
(function (global){
'use strict';
